...
WORKDIR /src
COPY ["FrontAndBackMerged.csproj", "FrontAndBackMerged/"]
RUN dotnet restore "FrontAndBackMerged/FrontAndBackMerged.csproj"

WORKDIR "/src/FrontAndBackMerged"
COPY . .
RUN dotnet build "FrontAndBackMerged.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "FrontAndBackMerged.csproj" \
...
Execute unit tests
```
cd "FrontAndBackMerged/test/FrontAndBackMerged.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "FrontAndBackMerged/src/FrontAndBackMerged"
