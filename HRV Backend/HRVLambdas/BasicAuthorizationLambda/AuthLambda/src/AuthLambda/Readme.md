...
WORKDIR /src
COPY ["AuthLambda.csproj", "AuthLambda/"]
RUN dotnet restore "AuthLambda/AuthLambda.csproj"

WORKDIR "/src/AuthLambda"
COPY . .
RUN dotnet build "AuthLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AuthLambda.csproj" \
...
Execute unit tests
```
cd "AuthLambda/test/AuthLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "AuthLambda/src/AuthLambda"
