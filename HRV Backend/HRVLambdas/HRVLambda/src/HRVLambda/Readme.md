...
WORKDIR /src
COPY ["HRVAuthorizationLambda.csproj", "HRVAuthorizationLambda/"]
RUN dotnet restore "HRVAuthorizationLambda/HRVAuthorizationLambda.csproj"

WORKDIR "/src/HRVAuthorizationLambda"
COPY . .
RUN dotnet build "HRVAuthorizationLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HRVAuthorizationLambda.csproj" \
...
Execute unit tests
```
cd "HRVAuthorizationLambda/test/HRVAuthorizationLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "HRVAuthorizationLambda/src/HRVAuthorizationLambda"
